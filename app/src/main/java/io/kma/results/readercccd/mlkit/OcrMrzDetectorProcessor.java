...
public class OcrMrzDetectorProcessor extends VisionProcessorBase<FirebaseVisionText>
{
    ...
    private static String REGEX_ID_LINE_1 = "[I1]{1}[A-Z<]{4}[A-Z0-9<]{9}";
    private static String REGEX_ID_LINE_2 = "[0-9]{7}[FM<]{1}[0-9]{7}";  //[A-Z<]{3}";
    ...
    protected void onSuccess(
        ...
        System.out.println( "Read: "+fullRead);
        ...
        Pattern patternIDLine1 = Pattern.compile(REGEX_ID_LINE_1);
        Matcher matcherIDLine1 = patternIDLine1.matcher(fullRead);
        Pattern patternIDLine2 = Pattern.compile(REGEX_ID_LINE_2);
        Matcher matcherIDLine2 = patternIDLine2.matcher(fullRead);
        boolean foundIDLine1 = matcherIDLine1.find();
        boolean foundIDLine2 = matcherIDLine2.find();
        System.out.println("fullREAD: "+fullRead);
        if (foundIDLine1)
            System.out.println("foundIDLine1 OK");
        if (foundIDLine2)
            System.out.println("foundIDLine2 OK");

        if (foundIDLine1 && foundIDLine2)
        {
            String line1ID = matcherIDLine1.group(0);
            String line2ID = matcherIDLine2.group(0);
            String documentNumber = line1ID.substring(5, 14);
            String dateOfBirthDay = line2ID.substring(0, 6);
            String expirationDate = line2ID.substring(8, 14);
