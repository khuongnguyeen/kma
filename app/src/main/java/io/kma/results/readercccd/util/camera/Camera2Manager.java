package io.kma.results.readercccd.util.camera;
...
public class Camera2Manager {
    ...
    private CameraManagerListener cameraManagerListener=null;

    private int mReductionSize=8;
     ...
     */
    private static final int MAX_PREVIEW_WIDTH = 2100;
    ...
    public Camera2Manager(Context mContext, AutoFitTextureView textureView, CameraManagerListener cameraManagerListener) {
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                mImageReader = ImageReader.newInstance(largest.getWidth()/mReductionSize, largest.getHeight()/mReductionSize,
            ...
            if(cameraManagerListener!=null){
                cameraManagerListener.onCamera2ApiNotSupported();
            }
        }
    }

    /**
     * Opens the camera specified by {@link Camera2Manager#mCameraId}.
     */
    private void openCamera(int width, int height) {
        if (ContextCompat.checkSelfPermission(mContext, Manifest.permission.CAMERA)
                != PackageManager.PERMISSION_GRANTED) {
            if(cameraManagerListener!=null){
                cameraManagerListener.onPermissionRequest();
            }
            return;
        }
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(mImageSurface, surface),
                    new CameraCaptureSession.StateCallback() {
                        ...
                        public void onConfigureFailed(
                            ...
                            if(cameraManagerListener!=null){
                                cameraManagerListener.onConfigurationFailed();
                            }
                        }
                    }, null
            );
