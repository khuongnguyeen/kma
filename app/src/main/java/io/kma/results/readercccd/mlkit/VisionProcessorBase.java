...
public abstract class VisionProcessorBase<T> implements VisionImageProcessor {
    ...
    @Override
    public void process(Frame frame, int rotation, OcrListener ocrListener)
    {
        if (shouldThrottle.get()) {
            return;
        }
        FrameMetadata frameMetadata =
                new FrameMetadata.Builder().setWidth(frame.getSize().width).setHeight(frame.getSize().height).setRotation(rotation).build();
        ...
        FirebaseVisionImage fbVisionImage = FirebaseVisionImage.fromByteArray( frame.getImage(),metadata);
        detectInVisionImage(fbVisionImage,frameMetadata,ocrListener);
